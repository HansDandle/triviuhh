...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default='0.0.0.0',
        help="hostname to bind on (default 0.0.0.0 = all interfaces)",
    )
    parser.add_argument(
        "--wshostname",
        type=str,
    ...
    parser.add_argument(
        ...
        help="A tab-separated or CSV file with questions and answers",
    )
    parser.add_argument(
        "--no-autoadvance",
        dest="autoadvance",
        action="store_false",
        help="Disable automatic timer-based game stage advancement",
    )
    parser.set_defaults(autoadvance=True)
    args = parser.parse_args()
    print(f'CLI Arguments: {args}')

    bind_ip = args.host
    # Determine the advertised LAN IP for QR code and websocket_ip.js.
    # When binding to 0.0.0.0 (or empty), detect the actual LAN IP so that
    # phones on the same WiFi can reach the server.
    if args.host in ('0.0.0.0', ''):
        ...
        my_ip = args.host
    print(f'Binding on {bind_ip}, advertising IP: {my_ip}')
    ...
    # start servers:
    wsserver = SimpleWebSocketServer(bind_ip, args.wsport,
                                     WSFakeageServer, selectInterval=0.05)
    wsserver.handleTick = handleTick

    httpserver = http.server.HTTPServer((bind_ip, args.httpport),
